...
const PoolRewardsModal: React.FC<PoolRewardsModalProps> = ({
  ...
  if (!notStability) {
    ...
    if (stabilityDeposit.preonReward.gt(Decimal.from(0))) {
      rewardTokens[Object.keys(rewards).length + 1] = {
        address: "0x",
        token: "PREON",
        troveBalance: 0,
        walletBalance: format(stabilityDeposit.preonReward),
        isStable: false,
        apr: 0,
        isVault: false,
        underlying: "",
  ...
  const dollarValue = (): number => {
    ...
    rewardTokens.map(({ address, token, walletBalance }) => {
      if (format(rewards[address]) !== 0 && token !== "PREON") {
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="lg">
      ...
      <ModalContent>
        <ModalHeader fontSize="2xl" pb={1}>
          <Text mr={1.5}>
            {!notStability ? "Stability Pool Rewards" : "Claim PREON Rewards"}{" "}
            {!notStability && (
              <Tooltip>
                Obtain collateral rewards from liquidations, and PREON rewards all the time
              </Tooltip>
            )}
          </Text>
        </ModalHeader>
              ...
              <TokenTable headers={["token", "amount"]} width={5}>
                <>
                  {rewardTokens.map(({ token, address, walletBalance }) => {
                    console.log("@farm: ", token, rewards[address]);
                    return (
                      (format(rewards[address]) !== 0 || token === "PREON") && (
                        <Tr key={token}>
                          <Td pb={0} pt={4}>
                            <Flex align="center">
                          ...
                          <Td pb={0} pt={4}>
                            {console.log("eeeeeee", format(walletBalance))}
                            <CoinAmount
                              token={token} //helpme
                              amount={
                                token === "PREON"
                ...
                <>
                  ...
                  {!notStability && dollarValue() !== 0 && (
                    <Tr key={"total"}>
                      ...
                      <Td pb={0} pt={4}>
                        <CoinAmount token={"STAR"} amount={dollarValue()} />
                      </Td>
                    </Tr>
                  )}
                </>
        ...
        <ModalFooter justifyContent={"flex-start"} mt={2}>
          <VStack align="stretch" spacing={6}>
            {/* {rewardTokens.length !== 0 && rewardTokens[0].token !== "PREON" && !notStability && (
              <HStack>
                <PoolSwap
                ...
                <Flex direction="column">
                  <Text textStyle="body2" fontWeight="bold">
                    ...
                    <Tooltip>
                      ...
                      swapping earned collateral back for STAR.
                    </Tooltip>
                    <HStack
                      marginTop={0.5}
                      spacing={2}
            ...
            {/* {rewardTokens.length !== 0 && rewardTokens[0].token !== "PREON" && !notStability && (
              <Text> Min STAR Expect to Receive: {getNum(dollarValue() * (1 - autoCompound(customValue, slippage)))}</Text>
            )} */}
            <Box>
              {mode !== "PREON" && mode !== "LP" ? (
                <Button variant="gradient" mr={6} onClick={onSubmit}>
                  ...
                  {
                    <Flex ml={1}>
                      <Tooltip>Claim all PREON and collateral rewards.</Tooltip>
                    </Flex>
                  }
                </Button>
              ) : (
