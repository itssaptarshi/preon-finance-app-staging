...
const selector = ({ starBalance, stabilityDeposit }: LiquityStoreState) => ({
  starBalance,
  stabilityDeposit
});
...
const ConfirmDepositModal: React.FC<ConfirmDepositModalProps> = ({
  ...
  const { starBalance, stabilityDeposit } = useLiquitySelector(selector);
  ...
  if (/^[0-9.]*$/.test(formatedAmount)) {
    if (mode === "deposit" && formatedAmount === format(starBalance)) {
      newAmount = starBalance;
    } else {
      if (
        mode === "withdraw" &&
        Decimal.from(formatedAmount).add(Decimal.from(".000009")).gte(stabilityDeposit.currentSTAR)
      ) {
        newAmount = stabilityDeposit.currentSTAR;
      } else if (
        mode === "deposit" &&
        Decimal.from(formatedAmount).add(Decimal.from(".000009")).gte(starBalance)
      ) {
        newAmount = starBalance;
      } else {
  ...
  const [sendTransaction] = useTransactionFunction(
    transactionId,
    mode === "deposit"
      ? liquity.send.depositSTARInStabilityPool.bind(liquity.send, newAmount)
      : liquity.send.withdrawSTARFromStabilityPool.bind(liquity.send, newAmount)
  );
      ...
      <ModalContent>
        ...
        <ModalBody>
          ...
          <Flex mt={3}>
            ...
            <Spacer />
            <CoinAmount amount={total} token="STAR" fontWeight="bold" />
          </Flex>
        </ModalBody>
