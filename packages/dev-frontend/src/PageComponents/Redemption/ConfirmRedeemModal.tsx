...
const selector = ({ starBalance, underlyingPrices }: LiquityStoreState) => ({
  starBalance,
  underlyingPrices
});
...
const ConfirmRedeemModal: React.FC<ConfirmRedeemModalProps> = ({
  ...
  const { starBalance, underlyingPrices } = useLiquitySelector(selector);
  ...
  if (
    formatedAmount === format(starBalance) ||
  ...
  const checkAllowance = async (token: string, amount: Decimal): Promise<boolean> => {
    const result = await liquity.getAllowanceOf(
      account,
      token,
      contractAddresses.boostedFarm.address, //boostedFarm
      amount
    );
  ...
  const [redeem] = useTransactionFunction(
    transactionId,
    liquity.send.redeemSTAR.bind(liquity.send, newAmount, redeemRate.add(Decimal.from(0.0005)))
  );
  ...
  const [approveTransaction] = useTransactionFunction(
    ...
    liquity.send.approveToken.bind(
      liquity.send,
      contractAddresses.starToken.address, // star token
      contractAddresses.boostedFarm.address, // boostedFarm
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="lg">
        ...
        <ModalContent>
          ...
          <ModalFooter flexDirection="column">
            <HStack spacing={6}>
              <Button variant={hasAllowance ? "quaternary" : "gradient"} onClick={onApprove}>
                Approve
              </Button>

              <Button
                variant={!hasAllowance ? "quaternary" : "gradient"}
