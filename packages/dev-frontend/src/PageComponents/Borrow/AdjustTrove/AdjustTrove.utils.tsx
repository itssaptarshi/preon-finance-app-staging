...
export const checkBorrow = (
  values: formValues,
  totalSTARFromLever: Decimal,
  debt: Decimal,
  ICR: number
) => {
  let newTotalBorrow =
    values["STARmode"] === "deposit"
      ? addString(format(debt), values["STAR"])
      : subtractString(format(debt), values["STAR"]);
  // todo check if star from lever deposit or withdraw
  newTotalBorrow = newTotalBorrow + Number(totalSTARFromLever.toString());
  ...
  if (values["STARmode"] === "deposit" && newTotalBorrow < minBorrow) {
    message = "STAR: Invalid Borrow";
    description = `You must borrow at least ${minBorrow} STAR`;
  } else if (ICR < 110) {
    message = "STAR: Invalid Borrow";
    description = `Your new total STAR borrow would make your collateral ratio ${ICR}%, which is below the minimum threshold of 110%.`;
  } else {
    borrowApproved = true;
  }
...
export const getAffectedCollateral = (values: formValues): AdjustedTokenData[] => {
  ...
  Object.keys(values).map(key => {
    if (key.includes("mode") && key.slice(0, -4) != "STAR") {
  ...
  tokenDataChanges.map(token => {
    ...
    token["starFromLever"] = values[token.token + "ExpectedSTAR"]
      ? values[token.token + "ExpectedSTAR"]
      : "0";
...
export const getTempAffectedCollateral = (values: formValues): AdjustedTokenData[] => {
  ...
  tokenDataChanges.map(token => {
    // console.log(values[token.token.concat("tempExpectedSTAR")])
    const change = values[token.token] ? parseFloat(String(values[token.token])) : 0;
    token["mode"] = values[token.token] ? String(values[token.token + "mode"]) : "deposit";
    token["change"] = change;
    token["starFromLever"] = values[token.token + "tempExpectedSTAR"]
      ? values[token.token + "tempExpectedSTAR"]
      : "0";
...
// TODO fix type defs
export const calculateTotalSTARFromLever = (
  affectedCollateral: AdjustedTokenData[],
  underlyingPrices: any, //TroveMappings,
  values: any,
  safetyRatios: any //TroveMappings
): Decimal => {
  let totalSTARFromLever: Decimal = Decimal.ZERO;
  if (affectedCollateral.length === 0) {
    return totalSTARFromLever;
  }
  // console.log('affectedCollateral', affectedCollateral)
  if (affectedCollateral[0].mode === "withdraw") {
    // console.log('hit 2')
    affectedCollateral.map(collateral => {
      const { starFromLever } = collateral;
      if (starFromLever != undefined && +starFromLever != 0) {
        collateral["starFromLever"] = String(
          Decimal.from(collateral.change).mul(underlyingPrices[collateral.address])
        );
        totalSTARFromLever = totalSTARFromLever.add(Decimal.from(collateral["starFromLever"]));
      }
      return totalSTARFromLever;
    });
  } else {
    ...
    affectedCollateral.map(collateral => {
      const { starFromLever } = collateral;
      // console.log('star inside', starFromLever)
      if (starFromLever != undefined && +starFromLever != 0) {
  ...
  // console.log('totalSTARFromLever', totalSTARFromLever)
  return totalSTARFromLever;
};
...
export const getTroveVaultValueWithLever = (
  ...
  return adjustValue(
    ...
    item.troveBalance,
    +amountToAdjust
    // (+amountToAdjust * format(1)).toString() // ! TODO: #pending #decimal works
  );
};
