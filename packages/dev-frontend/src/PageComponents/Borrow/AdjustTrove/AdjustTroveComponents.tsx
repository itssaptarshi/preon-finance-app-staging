...
const AdjustTroveComponents: React.FC<AdjustTroveComponentsProps> = ({
  ...
  const validate = (
    ...
    try {
      ...
      let totalSTARFromLever: Decimal = calculateTotalSTARFromLever(
        adjustedCollateral,
        underlyingPrices,
        values,
        safetyRatios
      ...
      if (troveBalancePost < 2000 && trove.status == "open") {
        toast({
          ...
          description:
            "You are paying back more STAR than minimum debt required to keep trove open (2000 STAR). Please reduce debt repayment amount or close trove.",
          ...toastProps
        });
        throw BreakException;
        ...
        toast({
          title: "Invalid Action",
          description: "You must borrow at least 2000 STAR in order to open a Trove.",
          ...toastProps
        });
        throw BreakException;
      }

      if (!changes && values["STAR"] === 0) {
        toast({
          title: "Invalid Action",
      ...
      if ((changes || values["STAR"] !== 0) && borrowApproved && collateralApproved) {
          ...
          if (values[token] !== 0) {
            const amount = Decimal.fromWithPrecision(
              isNaN(valuesWithFee) || valuesWithFee < 0
                ? 0
                : // : +valuesWithFee * format(receiptPerUnderlyingRatios[address]), // !TODO: #pending: gives error as it makes every token 1e18 #ratio
                  +valuesWithFee,
              dec
            );
  ...
  if (values["STARmode"] !== undefined) {
    coins["STAR"] = values["STARmode"];
  } else {
    coins["STAR"] = "deposit";
  }
  ...
  return (
    <>
      ...
      {/* <Flex align="center" mt={4} mx={6}>
              <Text textStyle="body2">STAR Borrow Fee: {(+borrowingRate.mul(100)).toFixed(3)}%</Text>
              <Spacer />
              <Button variant="gradient" onClick={() => validate(values)}>
                Confirm Changes
              </Button>
            </Flex> */}
      {isCapExceed ? (
