...
const Trove: React.FC<TroveProps> = ({ disconnected = false, borrowMode, setBorrowMode }) => {
  ...
  if (trove && trove.status !== "open") {
    return (
      <Box layerStyle="base" bg="brand.1100" flex={1}>
        <Text textStyle="title2">Trove</Text>
        <Text textStyle="title4" mt={2} color="purple">
      ...
      <Flex flex={1} direction={["column", null, "row"]}>
        <Flex direction="column" flex={3} mr={[0, null, 5]}>
          ...
          >
            ...
            <Spacer />
            <Text textStyle="subtitle2" color="purple">
              {getNum(vcValue, 2)} RAV (${getNum(usdValue, 2)})
            </Text>
          </Flex>

          {/* Total borrowed */}
          <Flex align="center" py={3} pl={2} pr={4} mb={2} borderRadius="2xl">
            <Flex align="center">
              <Icon iconName="Bank" w={12} h={5} />
              <Text textStyle="subtitle3">Total Borrowed</Text>
            </Flex>
            <Spacer />
            <Text mr={20} textStyle="subtitle2" color="purple">
              {getNum(totalBorrowed)} STAR
            </Text>
          </Flex>
        </Flex>
        ...
        <Flex direction="column" flex={2} mt={[6, null, 0]} ml={[0, null, 5]}>
          ...
          <Flex direction="column" mb={3}>
            ...
            <Flex gap={2} mb={1} mt={4}>
              <Flex>
                <Text textStyle="title4" color="White">
                  Collateral Ratio:{" "}
                </Text>
              </Flex>
              <Flex>
