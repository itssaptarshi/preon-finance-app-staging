...
const LeverUpModal: React.FC<LeverUpModalProps> = ({
  ...
  const [troveBalancePost, setTroveBalancePost] = useState<number>(
    troveBalance + +(values["STAR"] === undefined ? String(0) : values["STAR"])
  );
  ...
  const updateTroveBalance = () => {
    ...
    let tempTroveBalancePost: number = troveBalance + (values["STAR"] ? +values["STAR"] : 0);
    ...
    let tempTotalSTARFromLever = calculateTotalSTARFromLever(
      adjustedCollateral,
      underlyingPrices,
      values,
      safetyRatios
    );
    tempTotalSTARFromLever = tempTotalSTARFromLever.add(
      Decimal.from(
        values[collateral.token.concat("tempExpectedSTAR")]
          ? values[collateral.token.concat("tempExpectedSTAR")]
          : 0
      )
    );
    // console.log("tempTotalSTARFromLever", tempTotalSTARFromLever.toString())
  ...
  // const liquidationPrice = (1.1 / icr) * Number(underlyingPrices[currAddress].toString()); // commented
  ...
  const onSubmit = () => {
    ...
    if (values[collateral.token + "tempExpectedSTAR"] != undefined) {
      values[collateral.token + "ExpectedSTAR"] = values[collateral.token + "tempExpectedSTAR"];
      delete values[collateral.token + "tempExpectedSTAR"];
    }
    ...
    // values[collateral.token + "tempExpectedSTAR"] = expectedSTAR
  ...
  function Unlever() {
    ...
    if (amountToLeverage!.gt(Decimal.ZERO)) {
      values[collateral.token + "tempExpectedSTAR"] = Number(amountToLeverage) * currPrice;
    } else {
      values[collateral.token + "tempExpectedSTAR"] = 0;
    }
    if (
      ...
      <>
        <Text textStyle="subtitle1" mb={5}>
          <Text as="span" fontWeight="normal">
            Expected STAR Repaid:
          </Text>{" "}
          {amountToLeverage.gt(Decimal.ZERO) ? Number(amountToLeverage) * currPrice : "0"} STAR
        </Text>
  ...
  function ExpectedLeverage() {
    ...
    values[collateral.token + "tempExpectedSTAR"] = expectedSTAR.toString();
    ...
    return (
      <>
        ...
        <Text textStyle="subtitle1" mb={5}>
          ...
          {expectedSTAR.gt(Decimal.ZERO) ? Number(expectedSTAR.toString()).toFixed(4) : "0"}
        </Text>
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="lg" closeOnOverlayClick={false}>
      ...
      <ModalContent>
        ...
        <ModalFooter justifyContent="flex-start">
          {values[collateral.token + "tempAmount"] ? (
            <Button variant="gradient" mr={6} onClick={onSubmit}>
              Save
            </Button>
          ) : (
            <Button variant="gradient" mr={6} onClick={onSubmit} isDisabled={true}>
              Save
            </Button>
          )}
