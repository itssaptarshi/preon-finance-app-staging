...
const ConfirmCloseTroveAutosellModal: React.FC<ConfirmCloseTroveAutosellModalProps> = ({
  ...
  const understandAutosellLabel =
    "I understand this will repay any STAR debt associated with the collateral I'm choosing to sell. Deposit fees will have to be paid to reopen a trove if all collateral is sold.";
  const confirmAutosellLabel =
    "I confirm that I want to auto-sell the chosen collateral and use it to repay STAR debt.";
  ...
  const { trove, underlyingPrices, starBalance, tokenBalances, decimals } = useLiquitySelector(
    selector
  );
  const { liquity } = useLiquity();
  tokenData.map(
    // ? TODO: #pending need to check this
    token =>
      ...
      <ModalContent>
        ...
        <ModalBody>
          ...
          <Flex mt={5} align="center">
            <Text textStyle="title4">
              Total Available STAR{" "}
              <Tooltip>Current STAR balance + STAR gained from auto-sell</Tooltip>
            </Text>
            <Spacer />

            <Text fontWeight="bold" fontSize="lg">
              {getNum(starReceived + starAvailable)} STAR
              {/* - 200 here for the STAR Gas compensation that will be paid back on close trove. */}
            </Text>
          </Flex>
          ...
          <Flex my={4} align="center">
            <Text textStyle="title4" color="green.400">
              STAR After Close Trove{" "}
            </Text>
            <Text ml={1} textStyle="title4">
              <Tooltip>
                STAR you will have in your wallet after the transaction (this is subject to
                slippage).
              </Tooltip>
            </Text>
        ...
        <ModalFooter flexDirection="column">
          <HStack spacing={6}>
            ...
            {/* <Button variant="noSTAR">
              Additional {getNum(totalDebt - starAvailable - 200, 2)} STAR required to close trove
            </Button> */}
          </HStack>
        </ModalFooter>
      </ModalContent>
