...
import avax from "../deployments/avalanche.json";
...
const deployments: {
  [chainId: number]: any | undefined;
} = {
  [avax.chainId]: avax,
  ...(dev !== null ? { [dev.chainId]: dev } : {})
};

const polygonDeployment: {
  [chainId: number]: any | undefined;
} = {
...
export interface EthersLiquityConnection extends EthersLiquityConnectionOptionalParams {
  totalStabilityPoolPREONReward: any;
  ...
  liquidityMiningPREONRewardRate;
...
const connectionFrom = (
  ...
  return branded({
    ...
    totalStabilityPoolPREONReward: Decimal.from(totalStabilityPoolPREONReward),
    liquidityMiningPREONRewardRate: Decimal.from(liquidityMiningPREONRewardRate),
    pngLiquidityMiningPREONRewardRate: Decimal.from(pngLiquidityMiningPREONRewardRate),
    tjLiquidityMiningPREONRewardRate: Decimal.from(tjLiquidityMiningPREONRewardRate),
    ...deployment,
    ...optionalParams
  });
};
...
export interface EthersLiquityConnectionOptionalParams {
   ...
   * @remarks
   * For example
   * {@link EthersLiquity.depositSTARInStabilityPool | depositSTARInStabilityPool(amount, frontendTag?)}
...
export function _connectByChainId(
  ...
  const deployment: _LiquityDeploymentJSON =
    ...
    // polygonDeployment?? panic(new UnsupportedNetworkError(chainId));
    polygonDeployment[chainId] ?? panic(new UnsupportedNetworkError(chainId));
