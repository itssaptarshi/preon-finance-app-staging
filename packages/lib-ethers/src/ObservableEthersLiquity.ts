...
export class ObservableEthersLiquity implements ObservableLiquity {
  ...
  watchSTARInStabilityPool(
    ...
    const { starToken, stabilityPool } = _getContracts(this._readable.connection);
    const { Transfer } = starToken.filters;

    const transferSTARFromStabilityPool = Transfer(stabilityPool.address);
    const transferSTARToStabilityPool = Transfer(null, stabilityPool.address);

    const stabilityPoolSTARFilters = [transferSTARFromStabilityPool, transferSTARToStabilityPool];

    const stabilityPoolSTARListener = debounce((blockTag: number) => {
      this._readable.getSTARInStabilityPool({ blockTag }).then(onSTARInStabilityPoolChanged);
    });

    stabilityPoolSTARFilters.forEach(filter => starToken.on(filter, stabilityPoolSTARListener));

    return () =>
      stabilityPoolSTARFilters.forEach(filter =>
        starToken.removeListener(filter, stabilityPoolSTARListener)
      );
  }
  ...
  watchSTARBalance(onSTARBalanceChanged: (balance: Decimal) => void, address?: string): () => void {
    ...
    const { Transfer } = starToken.filters;
    const transferSTARFromUser = Transfer(address);
    const transferSTARToUser = Transfer(null, address);

    const starTransferFilters = [transferSTARFromUser, transferSTARToUser];

    const starTransferListener = debounce((blockTag: number) => {
      this._readable.getSTARBalance(address, { blockTag }).then(onSTARBalanceChanged);
    });

    starTransferFilters.forEach(filter => starToken.on(filter, starTransferListener));

    return () =>
      starTransferFilters.forEach(filter => starToken.removeListener(filter, starTransferListener));
  }
}
