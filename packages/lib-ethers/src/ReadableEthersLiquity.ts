...
import { _getPreonVaultToken, _getERC20Token, _getLPToken } from "./contracts";
...
export class ReadableEthersLiquity implements ReadableLiquity {
  ...
  async getTotalRedistributed(overrides?: EthersCallOverrides): Promise<Trove> {
    const { troveManager, preonController } = _getContracts(this.connection);
    ...
    for (let i = 0; i < troveColls[0].length; i++) {
      const [collateral, debt] = await Promise.all([
        ...
        troveManager.getL_STAR(troveColls[0][i], { ...overrides }).then(decimalify)
      ]);
  ...
  async getTroveBeforeRedistribution(
    ...
    let snapshotSTARDebt = BigNumber.from("0");
    const totalSnapshotSTARDebt: any = {};
    // Flatten promises
    ...
    const getRewardSnapshotSTAR = troveColls[0].map((coll: any) => {
      return troveManager.getRewardSnapshotSTAR(address == undefined ? "" : address, coll);
    });
    promises = promises.concat(rewardColl);
    promises = promises.concat(getRewardSnapshotSTAR);
    ...
    for (let i = 0; i < troveColls[0].length; i++) {
      ...
      snapshotSTARDebt = snapshotSTARDebt.add(result[i * 3 + 2]);
    }
    // for (const key in collaterals) {
    //   const [stake, snapshotAmount, colSTARDebt] = await Promise.all([
    //     troveManager.getTroveStake(address, key),
    //     troveManager.getRewardSnapshotColl(address, key),
    //     troveManager.getRewardSnapshotSTAR(address, key)
    //   ])
    //   stakes[key] = decimalify(stake)
    //   snapshotAmounts[key] = decimalify(snapshotAmount);
    //   snapshotSTARDebt = snapshotSTARDebt.add(colSTARDebt);
    // }
    totalSnapshotSTARDebt["debt"] = decimalify(snapshotSTARDebt);
    if (numberify(troveStatus) == active) {
      return new TroveWithPendingRedistribution(
        ...
        debt,
        stakes,
        new Trove(snapshotAmounts, decimals, totalSnapshotSTARDebt)
      );
    } else {
      return new TroveWithPendingRedistribution(
        address,
  ...
  getCollPrice(address: any, overrides?: any): any {
    const { preonController } = _getContracts(this.connection);
    return preonController.getPrice(address, { ...overrides }).then(decimalify);
  }
  async getPrices(addresses: any, overrides?: any): Promise<any> {
    const { preonController } = _getContracts(this.connection);
    const prices: any = {};
    const promises = addresses.map((coll: any) => {
      return preonController.getPrice(coll);
    });
  ...
  async _getActivePool(overrides?: EthersCallOverrides): Promise<Trove> {
    ...
    const [activeDebt] = await Promise.all(
      [activePool.getSTARDebt({ ...overrides })].map(getBigNumber => getBigNumber.then(decimalify))
    );
  ...
  async _getDefaultPool(overrides?: EthersCallOverrides): Promise<Trove> {
    ...
    const [defaultDebt] = await Promise.all(
      [defaultPool.getSTARDebt({ ...overrides })].map(getBigNumber => getBigNumber.then(decimalify))
    );
  ...
  async getStabilityDeposit(
    ...
    const [initialValue, currentSTAR, collateralsGain, preonReward] = await Promise.all([
      stabilityPool.deposits(address, { ...overrides }),
      stabilityPool.getCompoundedSTARDeposit(address, { ...overrides }),
      stabilityPool.getDepositorGains(address, { ...overrides }),
      stabilityPool.getDepositorPREONGain(address, { ...overrides })
    ]);
    const collGain = convertToMap(collateralsGain[0], bntoDec(collateralsGain[1]));
    return new StabilityDeposit(
      // @ts-expect-error: Testing
      decimalify(initialValue),
      decimalify(currentSTAR),
      collGain,
      decimalify(preonReward)
    );
  }
  ...
  async getFarm(address: any, overrides?: any): Promise<any> {
    ...
    const [lpTokenBalance, earnedPREON, totalLPStaked, rewardRate] = await Promise.all([
  ...
  async getBoostedFarm(address: any, overrides?: any): Promise<any> {
    address !== null && address !== void 0 ? address : (address = _requireAddress(this.connection));
    const { boostedFarm, preonToken } = _getContracts(this.connection);
    const [lpTokenBalance, earnedPREON, totalLPStaked, rewardRate] = await Promise.all([
      boostedFarm.balanceOf(address, { ...overrides }),
      boostedFarm.earned(preonToken.address, address, { ...overrides }), // preon token #address
      boostedFarm.totalSupply({ ...overrides }),
      boostedFarm.rewardRate(preonToken.address, { ...overrides }) // preon token #address
    ]);
    console.log(
      "@getBoostedFarm",
      "lptoken balance:",
  ...
  async getVEPREONStake(address: any, overrides?: any): Promise<any> {
    address !== null && address !== void 0 ? address : (address = _requireAddress(this.connection));
    console.log(address);
    const { vePREON, vePREONEmissions } = _getContracts(this.connection);
    const [
      account,
      unlock,
      claim,
      ...
      BigNumber.from(0),
      BigNumber.from(0)
      // vePREON.balanceOf(address, { ...overrides }),
      // vePREON.unlock(address, { ...overrides }),
      // vePREON.claim({ ...overrides })
    ]);

    return new vePREONStake(
      decimalify(account),
      decimalify(unlock),
      decimalify(claim),
      decimalify(lockedEnd),
      decimalify(balanceOf),
      decimalify(balanceOfNFT),
      decimalify(ownerOf),
      decimalify(getLastUserSlope),
      decimalify(findBlockEpoch)
      // decimalify(tokenURI),
      // decimalify(totalSupply),
      // decimalify(adjustToDistribute)
    );
  }
  ...
  async getPoolRewardRate(overrides?: any): Promise<any> {
    ...
    const rewardRate = decimalify(
      await boostedFarm.rewardRate(preonToken.address, { ...overrides })
    );
    console.log("@getBoostedFarm reward rate:", +rewardRate);
    return rewardRate;
  }

  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getRemainingStabilityPoolPREONReward} */
  async getRemainingStabilityPoolPREONReward(overrides?: any): Promise<any> {
    const { communityIssuance } = _getContracts(this.connection);
    const issuanceCap = this.connection.totalStabilityPoolPREONReward;
    //TODO Preon
    const totalPREONIssued = decimalify(await communityIssuance.totalPreonIssued({ ...overrides }));
    const burnedRewards = 18000000;
    // totalPREONIssued approaches but never reaches issuanceCap
    return issuanceCap.sub(totalPREONIssued);
  }
  ...
  getSTARInStabilityPool(overrides?: EthersCallOverrides): Promise<Decimal> {
    const { stabilityPool } = _getContracts(this.connection);
    return stabilityPool.getTotalSTARDeposits({ ...overrides }).then(decimalify);
  }
  ...
  async getDepositFee(
    ...
    const { borrowerOperations, preonController, activePool, defaultPool } = _getContracts(
      this.connection
    );
    ...
    for (const coll in VCInput) {
      console.log("@getDepositfee: VCInput[coll].hex:", VCInput[coll].hex);
      console.log("@getDepositfee: VCInput[coll]:", +VCInput[coll]);
      const [collateralVCInput] = await Promise.all([
        preonController.getValueVC(coll, VCInput[coll].hex)
      ]);
      allVCDifference.add(collateralVCInput);
    }
    for (const coll in VCOutput) {
      const [collateralVCInput] = await Promise.all([
        preonController.getValueVC(coll, VCOutput[coll].hex)
      ]);
    ...
    for (const coll in VCInput) {
      ...
      const [collateralVCInput, activePoolVCBalance, defaultPoolVCBalance] = await Promise.all([
        await preonController.getValueVC(coll, VCInput[coll].hex),
      ...
      if (collateralVCInput.eq(BigNumber.from("0"))) {
        ...
        console.log("@getDepositFee collateralVCInput", collateralVCInput);
        console.log(
          "@getDepositFee activePoolVCBalance.add(defaultPoolVCBalance)",
          activePoolVCBalance.add(defaultPoolVCBalance)
        );
        console.log("@getDepositFee totalVCBalancePre", totalVCBalancePre);
        console.log(
          "@getDepositFee totalVCBalancePre.add(allVCDifference)",
          totalVCBalancePre.add(allVCDifference)
        );
        const [depositFee] = await Promise.all([
          preonController.getVariableDepositFee(
            coll,
            collateralVCInput,
            activePoolVCBalance.add(defaultPoolVCBalance),
            totalVCBalancePre,
  ...
  async getEstimatedVePreonRewards(amount: any, time: any, overrides?: any): Promise<any> {
    const { vePREONEmissions } = _getContracts(this.connection);
    const estimatedRewards = await vePREONEmissions.rewardToEarn(
      Decimal.from(String(amount)).hex,
      Decimal.from(String(time)).hex
    );
    return decimalify(estimatedRewards).div(Decimal.from(1000000000000000000));
  }
  async getEstimatedPREONPoolRewards(amount: any, time: any, overrides?: any): Promise<any> {
    const { stabilityPool } = _getContracts(this.connection);
    const estimatedRewards = await stabilityPool.getEstimatedPREONPoolRewards(
      Decimal.from(String(amount)).hex,
      Decimal.from(String(time)).hex
    );
    return decimalify(estimatedRewards).div(Decimal.from(1000000000000000000));
  ...
  async getStarAllowance(from: any, to: any, amount: any): Promise<any> {
    const { starToken } = _getContracts(this.connection);
    let result = false;
    const allowanceResult = await starToken.allowance(from, to);
    if (decimalify(allowanceResult).gte(amount)) {
      result = true;
    }
    return result;
  }
  // @DEV Revert to fetchPrice_v
  async getPREONPrice(): Promise<any> {
    const { priceFeed } = _getContracts(this.connection);
    console.log("@priceFeed: PreonPriceFeed : ", (await priceFeed.fetchPrice_v()).toString());
    // @ts-expect-error: testing
    return priceFeed.fetchPrice_v().then(decimalify);
  }
  // @DEV Revert to fetchPrice_v
  async getSTARPrice(): Promise<any> {
    const { STARPriceFeed } = _getContracts(this.connection);
    console.log("@priceFeed: STARPriceFeed : ", (await STARPriceFeed.fetchPrice_v()).toString());
    return STARPriceFeed.fetchPrice_v().then(decimalify);
  }
  ...
  async getReceiptPerUnderlying(address: any): Promise<any> {
    const signer = _requireSigner(this.connection);
    const vault = _getPreonVaultToken(address, signer);
  ...
  getSTARBalance(address?: string, overrides?: EthersCallOverrides): Promise<Decimal> {
    address ??= _requireAddress(this.connection);
    const { starToken } = _getContracts(this.connection);

    return starToken.balanceOf(address, { ...overrides }).then(decimalify);
  }
  /** {@inheritDoc @liquity/lib-base#ReadableLiquity.getPREONBalance} */
  // @ts-expect-error: Testing
  getPREONBalance(address: any, overrides: { blockTag: any }): any {
    address !== null && address !== void 0 ? address : (address = _requireAddress(this.connection));
    const { preonToken } = _getContracts(this.connection);
    const balance = preonToken.balanceOf(address, { ...overrides }).then(decimalify);
    return balance;
  }
  ...
  async getVaultTokens(whitelistedCollaterals: string | any[], overrides?: any): Promise<any> {
    ...
    for (let i = 0; i < whitelistedCollaterals.length; i++) {
      ...
      if (await preonController.isWrapped(whitelistedCollaterals[i], { ...overrides })) {
  ...
  async getWhitelistedCollaterals(overrides?: any | undefined): Promise<any> {
    const { preonController } = _getContracts(this.connection);
    return preonController.getValidCollateral({ ...overrides });
  }
  // @ts-expect-error: Testing
  ...
  async getSafetyRatios(addresses: any[], overrides?: any): Promise<any> {
    ...
    const promises = addresses.map((coll: any) => {
      return preonController.getSafetyRatio(coll);
    });
    const result = await Promise.all(promises);
    for (let i = 0; i < addresses.length; i++) {
  ...
  async getRecoveryRatios(addresses: any[], overrides?: any): Promise<any> {
    ...
    const promises = addresses.map((coll: any) => {
      return preonController.getRecoveryRatio(coll);
    });
    const result = await Promise.all(promises);
    for (let i = 0; i < addresses.length; i++) {
  ...
  async getDecimals(addresses: any, overrides?: any): Promise<any> {
    ...
    const decimals: any = {};
    const promises = addresses.map((coll: any) => {
      return preonController.getDecimals(coll);
    });
    const result: BigNumber[] = await Promise.all(promises);
    for (let i = 0; i < addresses.length; i++) {
  ...
  troves.map(
    trove =>
      new TroveWithPendingRedistribution(
        ...
        //check whether we should return total snapshot STAR debt instead of per collateral
        new Trove(
          convertToMap(trove.allColls, trove.snapshotAmounts),
          decimals,
          convertToMap(trove.allColls, trove.snapshotSTARDebts)
        )
      )
  );
...
class _BlockPolledReadableEthersLiquity
  ...
  async getVEPREONStake(address: any, overrides: any): Promise<any> {
    return this._userHit(address, overrides)
      ? this.store.state.vePREONStaked
      : this._readable.getVEPREONStake(address, overrides);
  }
  async getPoolRewardRate(overrides: any): Promise<any> {
    return this._blockHit(overrides)
  ...
  async getRemainingStabilityPoolPREONReward(overrides: any): Promise<any> {
    return this._blockHit(overrides)
      ? this.store.state.remainingStabilityPoolPREONReward
      : this._readable.getRemainingStabilityPoolPREONReward(overrides);
  }

  async getSTARInStabilityPool(overrides?: EthersCallOverrides): Promise<Decimal> {
    return this._blockHit(overrides)
      ? this.store.state.starInStabilityPool
      : this._readable.getSTARInStabilityPool(overrides);
  }
  ...
  async getSTARBalance(address: any, overrides: any): Promise<any> {
    return this._userHit(address, overrides)
      ? this.store.state.starBalance
      : this._readable.getSTARBalance(address, overrides);
  }

  async getPREONBalance(address: any, overrides: any): Promise<any> {
    return this._userHit(address, overrides)
      ? this.store.state.preonBalance
      : this._readable.getPREONBalance(address, overrides);
  }
  async getCollateralSurplusBalancee(overrides: any): Promise<any> {
    return this._blockHit(overrides)
