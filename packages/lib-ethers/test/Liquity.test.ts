...
describe("EthersLiquity", () => {
  ...
  describe("findHintForCollateralRatio", () => {
    it("should pick the closest approx hint", async () => {
      ...
      const params = Trove.recreate(new Trove(Decimal.from(1), STAR_MINIMUM_DEBT));
  ...
  describe("Trove", () => {
    ...
    it("should fail to create an undercollateralized Trove", async () => {
      const price = await liquity.getPrice();
      const undercollateralized = new Trove(STAR_MINIMUM_DEBT.div(price), STAR_MINIMUM_DEBT);
    ...
    it("should fail to create a Trove with too little debt", async () => {
      const withTooLittleDebt = new Trove(Decimal.from(50), STAR_MINIMUM_DEBT.sub(1));

      await expect(liquity.openTrove(Trove.recreate(withTooLittleDebt))).to.eventually.be.rejected;
    });

    const withSomeBorrowing = { depositCollateral: 50, borrowSTAR: STAR_MINIMUM_NET_DEBT.add(100) };
    ...
    it("should create a Trove with some borrowing", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(withSomeBorrowing.borrowSTAR)}`);
    });
    ...
    const repaySomeDebt = { repaySTAR: 10 };

    it("should repay some debt", async () => {
      const { newTrove, fee } = await liquity.repaySTAR(repaySomeDebt.repaySTAR);
      expect(newTrove).to.deep.equal(Trove.create(withSomeBorrowing).adjust(repaySomeDebt));
      expect(`${fee}`).to.equal("0");
    });

    const borrowSomeMore = { borrowSTAR: 20 };

    it("should borrow some more", async () => {
      const { newTrove, fee } = await liquity.borrowSTAR(borrowSomeMore.borrowSTAR);
      expect(newTrove).to.deep.equal(
        Trove.create(withSomeBorrowing).adjust(repaySomeDebt).adjust(borrowSomeMore)
      );
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowSomeMore.borrowSTAR)}`);
    });
    ...
    const repayAndWithdraw = { repaySTAR: 60, withdrawCollateral: 0.5 };
    ...
    const borrowAndDeposit = { borrowSTAR: 60, depositCollateral: 0.5 };
    ...
    it("should borrow more and deposit some collateral at the same time", async () => {
      ...
      expect(`${fee}`).to.equal(`${MINIMUM_BORROWING_RATE.mul(borrowAndDeposit.borrowSTAR)}`);
    ...
    it("should close the Trove with some STAR from another user", async () => {
      const price = await liquity.getPrice();
      const initialTrove = await liquity.getTrove();
      const starBalance = await liquity.getLQTYBalance();
      const starShortage = initialTrove.netDebt.sub(starBalance);

      let funderTrove = Trove.create({ depositCollateral: 1, borrowSTAR: starShortage });
      funderTrove = funderTrove.setDebt(Decimal.max(funderTrove.debt, STAR_MINIMUM_DEBT));
      ...
      await funderLiquity.sendSTAR(await user.getAddress(), starShortage);

      const { params } = await liquity.closeTrove();

      expect(params).to.deep.equal({
        withdrawCollateral: initialTrove.collateral,
        repaySTAR: initialTrove.netDebt
      });
  ...
  describe("SendableEthersLiquity", () => {
    it("should parse failed transactions without throwing", async () => {
      ...
      const tx = await liquity.send.openTrove(
        { depositCollateral: 0.01, borrowSTAR: 0.01 },
        undefined,
        { gasLimit: 1e6 }
      );
  ...
  describe("Frontend", () => {
    ...
    it("other user's deposit should be tagged with the frontend's address", async () => {
      ...
      await otherLiquity.openTrove({ depositCollateral: 20, borrowSTAR: STAR_MINIMUM_DEBT });

      await otherLiquity.depositSTARInStabilityPool(STAR_MINIMUM_DEBT);
  ...
  describe("StabilityPool", () => {
    before(async () => {
      ...
      await funder.sendTransaction({
        to: otherUsers[0].getAddress(),
        value: STAR_MINIMUM_DEBT.div(170).hex
      });
    });

    const initialTroveOfDepositor = Trove.create({
      depositCollateral: STAR_MINIMUM_DEBT.div(100),
      borrowSTAR: STAR_MINIMUM_NET_DEBT
    });
    ...
    it("should make a small stability deposit", async () => {
      ...
      const details = await liquity.depositSTARInStabilityPool(smallStabilityDeposit);

      expect(details).to.deep.equal({
        starLoss: Decimal.from(0),
        newSTARDeposit: smallStabilityDeposit,
        collateralGain: Decimal.from(0),
        lqtyReward: Decimal.from(0),

        change: {
          depositSTAR: smallStabilityDeposit
        }
      });
    });

    const troveWithVeryLowICR = Trove.create({
      depositCollateral: STAR_MINIMUM_DEBT.div(180),
      borrowSTAR: STAR_MINIMUM_NET_DEBT
    });
    ...
    it("should liquidate other user's Trove", async () => {
      ...
      expect(details).to.deep.equal({
        ...
        starGasCompensation: STAR_LIQUIDATION_RESERVE,
    ...
    it("should transfer the gains to the Trove", async () => {
      ...
      expect(details).to.deep.equal({
        starLoss: smallStabilityDeposit,
        newSTARDeposit: Decimal.ZERO,
    ...
    describe("when people overstay", () => {
      before(async () => {
        ...
        // Use this account to print STAR
        await liquity.openTrove({ depositCollateral: 50, borrowSTAR: 5000 });

        // otherLiquities[0-2] will be independent stability depositors
        await liquity.sendSTAR(await otherUsers[0].getAddress(), 3000);
        await liquity.sendSTAR(await otherUsers[1].getAddress(), 1000);
        await liquity.sendSTAR(await otherUsers[2].getAddress(), 1000);

        // otherLiquities[3-4] will be Trove owners whose Troves get liquidated
        await otherLiquities[3].openTrove({ depositCollateral: 21, borrowSTAR: 2900 });
        await otherLiquities[4].openTrove({ depositCollateral: 21, borrowSTAR: 2900 });

        await otherLiquities[0].depositSTARInStabilityPool(3000);
        await otherLiquities[1].depositSTARInStabilityPool(1000);
        ...
        await otherLiquities[2].depositSTARInStabilityPool(1000);
        ...
        expect(`${await liquity.getSTARInStabilityPool()}`).to.equal("0");
      });
      ...
      it("should still be able to withdraw remaining deposit", async () => {
        for (const l of [otherLiquities[0], otherLiquities[1], otherLiquities[2]]) {
          ...
          await l.withdrawSTARFromStabilityPool(stabilityDeposit.currentSTAR);
        }
      });
    });
  });
  ...
  describe("Redemption", () => {
    const troveCreations = [
      { depositCollateral: 99, borrowSTAR: 4600 },
      { depositCollateral: 20, borrowSTAR: 2000 }, // net debt: 2010
      { depositCollateral: 20, borrowSTAR: 2100 }, // net debt: 2110.5
      { depositCollateral: 20, borrowSTAR: 2200 } //  net debt: 2211
    ];
    ...
    it("should fail to redeem during the bootstrap phase", async () => {
      ...
      await expect(liquity.redeemSTAR(4326.5)).to.eventually.be.rejected;
    });

    const someSTAR = Decimal.from(4326.5);

    it("should redeem some STAR after the bootstrap phase", async () => {
      ...
      const expectedDetails = {
        attemptedSTARAmount: someSTAR,
        actualSTARAmount: someSTAR,
        collateralTaken: someSTAR.div(200),
        fee: new Fees(0, 0.99, 2, new Date(), new Date(), false)
          .redemptionRate(someSTAR.div(total.debt))
          .mul(someSTAR.div(200))
      };

      const { rawReceipt, details } = await waitForSuccess(liquity.send.redeemSTAR(someSTAR));
      ...
      expect(`${await liquity.getSTARBalance()}`).to.equal("273.5");
    ...
    it("borrowing rate should be maxed out now", async () => {
      const borrowSTAR = Decimal.from(10);

      const { fee, newTrove } = await liquity.borrowSTAR(borrowSTAR);
      expect(`${fee}`).to.equal(`${borrowSTAR.mul(MAXIMUM_BORROWING_RATE)}`);

      expect(newTrove).to.deep.equal(
        Trove.create(troveCreations[0]).adjust({ borrowSTAR }, MAXIMUM_BORROWING_RATE)
      );
    });
  });

  describe("Redemption (truncation)", () => {
    const troveCreationParams = { depositCollateral: 20, borrowSTAR: 2000 };
    const netDebtPerTrove = Trove.create(troveCreationParams).netDebt;
    const amountToAttempt = Decimal.from(3000);
    const expectedRedeemable = netDebtPerTrove.mul(2).sub(STAR_MINIMUM_NET_DEBT);
    ...
    beforeEach(async () => {
      ...
      await liquity.openTrove({ depositCollateral: 99, borrowSTAR: 5000 });
    ...
    it("should truncate the amount if it would put the last Trove below the min debt", async () => {
      const redemption = await liquity.populate.redeemSTAR(amountToAttempt);
      expect(`${redemption.attemptedSTARAmount}`).to.equal(`${amountToAttempt}`);
      expect(`${redemption.redeemableSTARAmount}`).to.equal(`${expectedRedeemable}`);
      expect(redemption.isTruncated).to.be.true;

      const { details } = await waitForSuccess(redemption.send());
      expect(`${details.attemptedSTARAmount}`).to.equal(`${expectedRedeemable}`);
      expect(`${details.actualSTARAmount}`).to.equal(`${expectedRedeemable}`);
    });

    it("should increase the amount to the next lowest redeemable value", async () => {
      const increasedRedeemable = expectedRedeemable.add(STAR_MINIMUM_NET_DEBT);

      const initialRedemption = await liquity.populate.redeemSTAR(amountToAttempt);
      const increasedRedemption = await initialRedemption.increaseAmountByMinimumNetDebt();
      expect(`${increasedRedemption.attemptedSTARAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${increasedRedemption.redeemableSTARAmount}`).to.equal(`${increasedRedeemable}`);
      expect(increasedRedemption.isTruncated).to.be.false;

      const { details } = await waitForSuccess(increasedRedemption.send());
      expect(`${details.attemptedSTARAmount}`).to.equal(`${increasedRedeemable}`);
      expect(`${details.actualSTARAmount}`).to.equal(`${increasedRedeemable}`);
    });

    it("should fail to increase the amount if it's not truncated", async () => {
      const redemption = await liquity.populate.redeemSTAR(netDebtPerTrove);
  ...
  describe("Redemption (gas checks)", function () {
    ...
    before(async function () {
      ...
      for (const otherLiquity of otherLiquities) {
        await otherLiquity.openTrove({
          ...
          borrowSTAR: amountToBorrowPerTrove
        });
      }
    ...
    it("should redeem using the maximum iterations and almost all gas", async () => {
      await liquity.openTrove({
        ...
        borrowSTAR: amountToRedeem
      });

      const { rawReceipt } = await waitForSuccess(liquity.send.redeemSTAR(amountToRedeem));
  ...
  // Test workarounds related to https://github.com/PreonMoney/preon-contracts/issues/600
  describe("Hints (adjustTrove)", () => {
    ...
    before(async () => {
      ...
      await openTroves(eightOtherUsers, [
        { depositCollateral: 30, borrowSTAR: 2000 }, // 0
        { depositCollateral: 30, borrowSTAR: 2100 }, // 1
        { depositCollateral: 30, borrowSTAR: 2200 }, // 2
        { depositCollateral: 30, borrowSTAR: 2300 }, // 3
        // Test 1:           30,             2400
        { depositCollateral: 30, borrowSTAR: 2500 }, // 4
        { depositCollateral: 30, borrowSTAR: 2600 }, // 5
        { depositCollateral: 30, borrowSTAR: 2700 }, // 6
        { depositCollateral: 30, borrowSTAR: 2800 } //  7
        // Test 2:           30,             2900
    ...
    it("should not traverse the whole list when bottom Trove moves", async () => {
      ...
      const targetTrove = Trove.create({ depositCollateral: 30, borrowSTAR: 2900 });
      const interferingTrove = Trove.create({ depositCollateral: 30, borrowSTAR: 3000 });
    ...
    it("should not traverse the whole list when lowering ICR of bottom Trove", async () => {
      ...
      const targetTrove = [
        Trove.create({ depositCollateral: 30, borrowSTAR: 3100 }),
        Trove.create({ depositCollateral: 30, borrowSTAR: 3200 })
      ];
  ...
  describe("Gas estimation", () => {
    ...
    before(async function () {
      ...
      await openTroves(fiveOtherUsers, [
        { depositCollateral: 20, borrowSTAR: 2040 },
        { depositCollateral: 20, borrowSTAR: 2050 },
        { depositCollateral: 20, borrowSTAR: 2060 },
        { depositCollateral: 20, borrowSTAR: 2070 },
        { depositCollateral: 20, borrowSTAR: 2080 }
      ]);
    ...
    it("should include enough gas for updating lastFeeOperationTime", async () => {
      await liquity.openTrove({ depositCollateral: 20, borrowSTAR: 2090 });

      // We just updated lastFeeOperationTime, so this won't anticipate having to update that
      // during estimateGas
      const tx = await liquity.populate.redeemSTAR(1);
    ...
    it("should include enough gas for one extra traversal", async () => {
      ...
      expect(adjustment.borrowSTAR).to.be.undefined;
      ...
      assertDefined(rudeCreation.borrowSTAR);
      const starShortage = rudeTrove.debt.sub(rudeCreation.borrowSTAR);

      await liquity.sendSTAR(await rudeUser.getAddress(), starShortage);
    ...
    it("should include enough gas for both when borrowing", async () => {
      ...
      const adjustment = trove.adjustTo(newTrove);
      expect(adjustment.borrowSTAR).to.not.be.undefined;
  ...
  describe("Gas estimation (LQTY issuance)", () => {
    ...
    it("should include enough gas for issuing LQTY", async function () {
      ...
      await liquity.openTrove({ depositCollateral: 40, borrowSTAR: 4000 });
      await liquity.depositSTARInStabilityPool(19);
      ...
      const deposit = await liquity.populate.depositSTARInStabilityPool(1);
      const withdraw = await liquity.populate.withdrawSTARFromStabilityPool(1);
      ...
      await deployerLiquity.depositSTARInStabilityPool(creation.borrowSTAR);
    ...
    before(async function () {
      ...
      await openTroves(
        ...
        someMoreUsers.map((_, i) => ({
          ...
          borrowSTAR: STAR_MINIMUM_NET_DEBT.add(i / 10)
        }))
      );
      ...
      await Promise.all(
        otherLiquities.map(async otherLiquity =>
          otherLiquity.sendSTAR(await user.getAddress(), await otherLiquity.getSTARBalance())
        )
      );
      ...
      const redeemedTroveDebt = await liquity
        .getSTARBalance()
        .then(x => x.div(10).add(STAR_LIQUIDATION_RESERVE));
      ...
      const { actualSTARAmount } = await liquity.redeemSTAR(redeemedTrove.netDebt);

      expect(`${actualSTARAmount}`).to.equal(`${redeemedTrove.netDebt}`);
