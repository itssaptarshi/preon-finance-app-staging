...
export class Fees {
   ...
   * the `when` parameter.
   *
   * To calculate the borrowing fee in STAR, multiply the borrowed STAR amount by the borrowing rate.
   *
   * @example
   * ```typescript
   * const fees = await liquity.getFees();
   *
   * const borrowedSTARAmount = 100;
   * const borrowingRate = fees.borrowingRate();
   * const borrowingFeeSTAR = borrowingRate.mul(borrowedSTARAmount);
   * ```
   */
   ...
   *
   * @param redeemedFractionOfSupply - The amount of STAR being redeemed divided by the total supply.
   ...
   * precise, it depends on the fraction of the redeemed amount compared to the total STAR supply,
   * which must be passed as a parameter.
   *
   * To calculate the redemption fee in STAR, multiply the redeemed STAR amount with the redemption
   * rate.
   *
   * @example
   * ```typescript
   ...
   *
   * const redeemedSTARAmount = Decimal.from(100);
   * const redeemedFractionOfSupply = redeemedSTARAmount.div(total.debt);
   * const redemptionRate = fees.redemptionRate(redeemedFractionOfSupply);
   * const redemptionFeeSTAR = redemptionRate.mul(redeemedSTARAmount);
   * ```
   */
