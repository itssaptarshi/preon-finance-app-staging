...
export interface TroveCreationDetails {
  ...
  /** Amount of STAR added to the Trove's debt as borrowing fee. */
  fee: Decimal;
}
...
/**
 * Details of a {@link TransactableLiquity.redeemSTAR | redeemSTAR()} transaction.
 *
 * @public
 */
export interface RedemptionDetails {
   ...
   *
   * @remarks
   * This can end up being lower than `attemptedSTARAmount` due to interference from another
...
/**
 * Details of a
 * {@link TransactableLiquity.depositSTARInStabilityPool | depositSTARInStabilityPool()} or
 * {@link TransactableLiquity.withdrawSTARFromStabilityPool | withdrawSTARFromStabilityPool()}
 * transaction.
 *
 * @public
 */
...
export interface TransactableLiquity {
  /**
   * Open a new Trove by depositing collateral and borrowing STAR.
   *
   ...
   * {@link @liquity/lib-base#STAR_MINIMUM_DEBT}.
   *
   * If `maxBorrowingRate` is omitted, the current borrowing rate plus 0.5% is used as maximum
   * acceptable rate.
   */
   ...
   *
   * @param amount - The amount of STAR to borrow.
   * @param maxBorrowingRate - Maximum acceptable
   *                           {@link @liquity/lib-base#Fees.borrowingRate | borrowing rate}.
   *
   * @throws
   ...
   * ```
   */
  borrowSTAR(amount: Decimalish, maxBorrowingRate?: Decimalish): Promise<TroveAdjustmentDetails>;

  /**
   * Adjust existing Trove by repaying some of its debt.
   *
   * @param amount - The amount of STAR to repay.
   *
   * @throws
   * Throws {@link TransactionFailedError} in case of transaction failure.
   *
   ...
   * ```
   */
  repaySTAR(amount: Decimalish): Promise<TroveAdjustmentDetails>;
   ...
   *
   * @param amount - Amount of STAR to add to new or existing deposit.
   ...
   *
   * @param amount - Amount of STAR to withdraw.
   *
   * @throws
   * Throws {@link TransactionFailedError} in case of transaction failure.
   *
   ...
   * {@link @liquity/lib-base#StabilityDeposit.lqtyReward | LQTY reward}.
   */
  withdrawSTARFromStabilityPool(amount: Decimalish): Promise<StabilityDepositChangeDetails>;
   ...
   * @param amount - Amount of STAR to send.
   *
   * @throws
   * Throws {@link TransactionFailedError} in case of transaction failure.
   */
   ...
   *
   * @param amount - Amount of STAR to be redeemed.
   ...
   */
  redeemSTAR(amount: Decimalish, maxRedemptionRate?: Decimalish): Promise<RedemptionDetails>;
   ...
   * {@link @liquity/lib-base#vePREONStake.collateralGain | collateral gain} and
   * {@link @liquity/lib-base#vePREONStake.starGain | STAR gain}.
   */
  ...
  /**
   * Withdraw {@link @liquity/lib-base#vePREONStake.collateralGain | collateral gain} and
   * {@link @liquity/lib-base#vePREONStake.starGain | STAR gain} from LQTY stake.
   *
   * @throws
   * Throws {@link TransactionFailedError} in case of transaction failure.
   */
  ...
  /**
   * Allow the liquidity mining contract to use Uniswap ETH/STAR LP tokens for
  ...
  /**
   * Stake Uniswap ETH/STAR LP tokens to participate in liquidity mining and earn LQTY.
   *
  ...
  /**
   * Withdraw Uniswap ETH/STAR LP tokens from liquidity mining.
   *
