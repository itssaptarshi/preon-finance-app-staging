 ...
 * @public
 */
export type Decimalish = Decimal | number | string | any;

/**
 * Fixed-point decimal bignumber with 18 digits of precision.
 *
 * @remarks
 * Used by Liquity libraries to precisely represent native currency (e.g. Ether), STAR and LQTY
...
export class Decimal {
  ...
  static _fromStringWithPrecision(representation: any, precision: number): Decimal {
    const digits = getDigits(precision);
    if (!representation || !representation.match(stringRepresentationFormat)) {
      throw new Error(`bad decimal forma : "${representation}"`);
    }
    if (representation.includes("e")) {
      // eslint-disable-next-line prefer-const
      let [coefficient, exponent] = representation.split("e");
  ...
  static fromWithPrecision(decimalish: Decimalish, precision: number): Decimal {
    switch (typeof decimalish) {
      ...
      case "string":
        return Decimal._fromStringWithPrecision(decimalish, precision);
      case "number":
        return Decimal._fromStringWithPrecision(decimalish.toString(), precision);
      default:
        throw new Error("invalid Decimalish value");
    }
  }
  ...
  toNumber(): number {
    return this._bigNumber.toNumber();
  }
