...
/** @internal */ export type _STARBorrowing<T> = { borrowSTAR: T };
/** @internal */ export type _STARRepayment<T> = { repaySTAR: T };
...
/** @internal */ export type _NoSTARBorrowing = Partial<_STARBorrowing<undefined>>;
/** @internal */ export type _NoSTARRepayment = Partial<_STARRepayment<undefined>>;
...
export type _DebtChange<T> =
  | (_STARBorrowing<T> & _NoSTARRepayment)
  | (_STARRepayment<T> & _NoSTARBorrowing);

/** @internal */
export type _NoDebtChange = _NoSTARBorrowing & _NoSTARRepayment;
 ...
 *     <td> borrowSTAR </td>
 *     <td> T </td>
 *     <td> The amount of STAR that's borrowed. </td>
 *   </tr>
 *
 * </table>
 *
 ...
 *     <td> repaySTAR? </td>
 *     <td> T </td>
 *     <td> <i>(Optional)</i> The amount of STAR that's repaid. </td>
 *   </tr>
 *
 * </table>
 *
 ...
 * least one.
 *
 * Defining both `depositCollateral` and `withdrawCollateral`, or both `borrowSTAR` and `repaySTAR`
 ...
 *     <td> borrowSTAR? </td>
 *     <td> T </td>
 *     <td> <i>(Optional)</i> The amount of STAR that's borrowed. </td>
 *   </tr>
 *
 *   <tr>
 *     <td> repaySTAR? </td>
...
const allowedTroveCreationKeys = ["decimals", "depositCollaterals", "borrowSTAR"];
...
const debtChangeFrom = ({ borrowSTAR, repaySTAR }: any): any => {
  if (borrowSTAR !== undefined && repaySTAR !== undefined) {
    throw new Error(
      "TroveAdjustmentParams: 'borrowSTAR' and 'repaySTAR' can't be present at the same time"
    );
  }
...
const troveAddCollLeverUpParamsFromEntries = (entries: any) => {
  ...
  if (missingKeys.length > 1 && missingKeys[0] != "borrowSTAR") {
  ...
  const { borrowSTAR } = params;
  if (collateralLeverup !== undefined && borrowSTAR !== undefined) {
    return { ...collateralLeverup, borrowSTAR };
  }
...
const troveWithdrawCollUnleverUpParamsFromEntries = entries => {
  ...
  const missingKeys = allowedTroveWithdrawCollUnleverUpKeys
    ...
    .map(k => `'${k}'`);
  if (missingKeys.length > 1 && missingKeys[0] != "repaySTAR") {
  ...
  const { repaySTAR } = params;
  if (collateralUnleverup !== undefined && repaySTAR !== undefined) {
    return { ...collateralUnleverup, repaySTAR };
  }
...
export class Trove {
  ...
  /**
   * Amount of STAR that must be repaid to close this Trove.
   *
   * @remarks
   * This doesn't include the liquidation reserve, which is refunded in case of normal closure.
   */
  get netDebt(): Decimal {
    if (this.debt["debt"].lt(STAR_LIQUIDATION_RESERVE)) {
      throw new Error(`netDebt should not be used when debt < ${STAR_LIQUIDATION_RESERVE}`);
    }
    return this.debt["debt"].sub(STAR_LIQUIDATION_RESERVE);
  }
  ...
  _debtChange({ debt }: Trove, borrowingRate: Decimalish): any {
    return debt["debt"].gt(this.debt["debt"])
      ? { borrowSTAR: { debt: unapplyFee(borrowingRate, debt["debt"].sub(this.debt["debt"])) } }
      : { repaySTAR: { debt: this.debt["debt"].sub(debt["debt"]) } };
  }
  ...
  whatChanged(that: Trove, borrowingRate = MINIMUM_BORROWING_RATE): any {
    ...
    if (this.isEmpty) {
      if (that.debt["debt"].lt(STAR_LIQUIDATION_RESERVE)) {
      ...
      return troveCreation({
        ...
        borrowSTAR: { debt: unapplyFee(borrowingRate, that.netDebt) }
      });
    }
    if (that.isEmpty) {
      return troveClosure(
        this.netDebt.nonZero
          ? { withdrawCollaterals: this.collaterals, repaySTAR: { debt: this.netDebt } }
  ...
  apply(change: any, borrowingRate = MINIMUM_BORROWING_RATE): any {
    ...
    switch (change.type) {
      ...
      case "creation": {
        ...
        const { depositCollaterals, borrowSTAR, decimals } = change.params;
        ...
        return new Trove(depositCollaterals, decimals, {
          debt: STAR_LIQUIDATION_RESERVE.add(applyFee(borrowingRate, borrowSTAR["debt"]))
        });
      }
      ...
      case "adjustment": {
        const {
          setToZero,
          params: { depositCollaterals, withdrawCollaterals, borrowSTAR, repaySTAR }
        } = change;
        ...
        const debtDecrease =
          repaySTAR !== null && repaySTAR !== void 0 ? repaySTAR : { debt: Decimal.ZERO };
        const debtIncrease = {
          debt: borrowSTAR ? applyFee(borrowingRate, borrowSTAR["debt"]) : Decimal.ZERO
        };
