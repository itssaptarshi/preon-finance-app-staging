...
Defining both `depositCollateral` and `withdrawCollateral`<!-- -->, or both `borrowSTAR` and `repaySTAR` at the same time is disallowed, and will result in a type-checking error.
...
<tr> <td> borrowSTAR? </td> <td> T </td> <td> <i>(Optional)</i> The amount of STAR that's borrowed. </td> </tr>

<tr> <td> repaySTAR? </td> <td> T </td> <td> <i>(Optional)</i> The amount of STAR that's repaid. </td> </tr>
