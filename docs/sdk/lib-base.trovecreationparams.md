...
```typescript
export declare type TroveCreationParams<T = unknown> = _CollateralDeposit<T> & _NoCollateralWithdrawal & _STARBorrowing<T> & _NoSTARRepayment;
```
...
<tr> <td> borrowSTAR </td> <td> T </td> <td> The amount of STAR that's borrowed. </td> </tr>
